...
| 2 | `announcements.json` に **mode: replace** の指定がある日 | 曜日画像を**出さず**、指定ファイルのみ |
| 3 | 上記以外 | 曜日画像（`monday.png` 〜 `sunday.png` ／祝日は `holiday.png`） |

**mode: add** の指定がある日は、上のどちらの場合でも**そのあとに追加で**投稿される。
...
当日と予告を、それぞれ登録する。**日付入りの文言**にしておけば同じファイルを両方に使える。

```json
{
  "start": "2026-11-03", "end": "2026-11-03",
  "media": "20261103-kyushin.mp4", "mode": "replace",
  "label": "11/3 終日休診（当日・曜日画像は出さない）"
},
{
  "start": "2026-10-27", "end": "2026-11-02",
  "media": "20261103-kyushin.mp4", "mode": "add",
  "label": "11/3 休診の予告（前の週から毎日、曜日画像のあとに）"
}
```

> かつて `closures.json` という終日休診専用の仕組みがあったが、
> **休診日ごとに別の動画を指定できない**（`closure-video.mp4` 固定）、
> **当日と予告で同じ動画を使い回す**、**予告は7日前から毎日で日数を選べない**、
> という制約があったため 2026-09-09 に廃止し、この方式に一本化した。
...
- **2つの設定ファイルの使い分け**
  ...
  - `announcements.json` … その日の投稿内容を**差し替える／追加する**（時間変更・休診の告知など）
- **終日休診の告知も `announcements.json` で行う**。当日を `mode: replace`、
  予告を出したい日を `mode: add` で登録する（下記「終日休診のとき」参照）
...
| `announcements.json` | 特定日の差し替え・追加投稿の設定 |
